...
export const DailyDigestPanel = ({ digest, loading, onOpen }) => {
  const { t } = useTranslation();
  if (loading) return <aside className="animate-pulse rounded-[24px] border border-border/60 bg-card/45 p-5"><div className="h-4 w-28 rounded bg-card" /><div className="mt-4 h-12 rounded bg-card" /></aside>;
  if (!digest?.content?.items?.length) return null;
  return <aside className="rounded-[24px] border border-violet-400/20 bg-violet-400/[0.06] p-5"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-violet-300"><Sparkles className="h-4 w-4" />{t("news.daily.title")}</div><p className="mt-3 text-sm font-semibold leading-6">{digest.content.headline}</p><p className="mt-2 text-xs text-muted-foreground">{t("news.daily.summary", { news: digest.content.counts.news, assets: digest.content.counts.assets })}</p><div className="mt-4 space-y-2">{digest.content.items.slice(0, 3).map((item) => <button key={item.id} type="button" onClick={() => onOpen(item.id)} className="block w-full truncate text-left text-xs text-muted-foreground transition hover:text-primary">{item.symbol ? `${item.symbol} · ` : ""}{item.title}</button>)}</div></aside>;
};

export const HistoricalImpactPanel = ({ symbol, impact, loading, onLoad }) => {
  const { t } = useTranslation();
  const values = [["1h", impact?.return1h, impact?.after1hPrice], ["24h", impact?.return24h, impact?.after24hPrice], ["7d", impact?.return7d, impact?.after7dPrice]];
  const formatPrice = (value) => value == null ? "—" : Number(value).toLocaleString(undefined, { maximumFractionDigits: 6 });
  return <section className="rounded-[24px] border border-border/60 bg-card/45 p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">{t("news.impact.eyebrow")}</p><h2 className="mt-1 font-semibold">{t("news.impact.title", { symbol })}</h2></div>{!impact ? <Button size="sm" variant="outline" onClick={onLoad} disabled={loading}>{loading ? t("common.states.loading") : t("news.impact.calculate")}</Button> : null}</div>{impact ? impact.sampleSize ? <><div className="mt-4 rounded-xl border border-border/50 bg-background/35 p-3"><p className="text-[10px] uppercase text-muted-foreground">{t("news.impact.before")}</p><p className="mt-1 font-mono text-sm font-semibold">{formatPrice(impact.beforePrice)}</p></div><div className="mt-2 grid grid-cols-3 gap-2">{values.map(([label, value, price]) => <div key={label} className="rounded-xl border border-border/50 bg-background/35 p-3"><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-1 font-mono text-sm font-semibold">{value == null ? "—" : `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`}</p><p className="mt-1 truncate font-mono text-[10px] text-muted-foreground">{formatPrice(price)}</p></div>)}</div><p className="mt-3 text-[11px] leading-5 text-muted-foreground">{t("news.impact.disclaimer", { count: impact.sampleSize })}</p></> : <p className="mt-4 text-sm text-muted-foreground">{t("news.impact.noData")}</p> : <p className="mt-4 text-xs leading-5 text-muted-foreground">{t("news.impact.description")}</p>}</section>;
};

export const EducationalExplanationPanel = ({ result, loading, onExplain }) => {
  const { t } = useTranslation();
  const explanation = result?.explanation;
  return <section className="rounded-[24px] border border-sky-400/20 bg-sky-400/[0.05] p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-sky-300">{t("news.ai.eyebrow")}</p><h2 className="mt-1 font-semibold">{t("news.ai.title")}</h2></div>{!explanation ? <Button size="sm" variant="outline" onClick={onExplain} disabled={loading}>{loading ? t("common.states.loading") : t("news.ai.explain")}</Button> : null}</div>{explanation ? <div className="mt-4 space-y-4 text-sm leading-6"><p>{explanation.summary}</p><div><p className="font-semibold">{t("news.ai.relevance")}</p><p className="text-muted-foreground">{explanation.whyRelevant}</p></div><div className="flex flex-wrap gap-2">{explanation.concepts?.map((concept) => <TopicBadge key={concept} topic={concept} />)}</div><div><p className="font-semibold">{t("news.ai.questions")}</p><ul className="mt-1 list-disc space-y-1 pl-5 text-muted-foreground">{explanation.questions?.map((question) => <li key={question}>{question}</li>)}</ul></div><p className="border-t border-border/50 pt-3 text-xs text-muted-foreground">{explanation.disclaimer}</p></div> : <p className="mt-4 text-xs leading-5 text-muted-foreground">{t("news.ai.description")}</p>}</section>;
};
