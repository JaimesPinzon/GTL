...
import { TrendingUp, TrendingDown, Landmark, Coins, Briefcase } from "lucide-react";
...
const SymbolIcon = ({ type }) => {
  ...
  if (type === "crypto") {
    return <Coins className="h-5 w-5 mr-2 text-yellow-400" />;
  }
  if (type === "index") {
...
const MarketSymbols = () => {
  const { symbols, setSelectedSymbol, selectedSymbol } = useTradingWorkspace();
  const { t } = useTranslation();

  return (
    <div className="glass-card rounded-lg p-4">
      <h2 className="text-xl font-bold mb-4">{t("markets")}</h2>
